...
const loginUser = async function (req, res) {
    try {
        const data = req.body;
        if (!Object.keys(data).length) return res.status(400).send({ status: false, message: "Invalid request parameters. Please provide login details" });

        // Extract parameter 
        let { email, password } = data

        // valitaion start to here 
        if (!email) return res.status(400).send({ status: false, message: "Please include a email" });
        if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)) return res.status(400).send({ status: false, message: "Enter valid email address." })
        if (!isValid(email)) return res.status(400).send({ status: false, message: "Email is required for login" });
