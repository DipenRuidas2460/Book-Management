...
const createBook = async function (req, res) {
    try {
        ...
        if (!isValid(title)) return res.status(400).send({ status: false, message: "Provide a Valid Title" })
        ...
        if (!isValid(excerpt)) return res.status(400).send({ status: false, message: "Provide a Valid Excerpt" })

        if (!userId) return res.status(400).send({ status: false, message: "You must enter userId" })
        if (!isValid(userId)) return res.status(400).send({ status: false, message: "userId should not be empty & put it in  a string" })
        ...
        if (!isValid(ISBN)) return res.status(400).send({ status: false, message: "ISBN should not be empty" })
        ...
        if (!isValid(category)) return res.status(400).send({ status: false, message: "category should not be empty & put it in a string" })

        if (!subcategory) return res.status(400).send({ status: false, message: "subcategory is required" })
        if (!isValid(subcategory)) return res.status(400).send({ status: false, message: "Subcategory should not be empty & put it in a string" })
...
const getBooks = async function (req, res) {
    try {
        ...
        if (query.userId) {
            if (!mongoose.isValidObjectId(query.userId)) return res.status(400).send({ status: false, message: "userId  is not valid " })
        }

        if (query.category) {
            if (!isValid(query.category)) return res.status(400).send({ status: false, message: "category should not be empty" })
        }

        if (query.subcategory) {
            if (!isValid(query.subcategory)) return res.status(400).send({ status: false, message: "subcategory should not be empty" })
        }
...
const updateById = async function (req, res) {
    try {
        ...
        if (excerpt) {
            if (!isValid(excerpt)) return res.status(400).send({ status: false, message: "excerpt should not be empty " })
        }
